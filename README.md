...
已启用、设有截止日且未关联出游的心愿，以心愿截止日作为预计出发日，只同步 `出门todo` 中的 `出门前七个月`（兼容 `出门前7个月`）及其后代任务。阶段根任务定在截止日前七个自然月，月底不足对应日时取当月末；子任务和 checklist 日期按该阶段模板相对平移。任务名称包含心愿名，按心愿 ID 更新改名和改期，保留完成状态；关联出游、停用或删除心愿后清理尚未到截止日的独立副本，日历完整出游任务不受影响。无截止日的心愿不生成，缺少唯一七个月模板时会报错。
