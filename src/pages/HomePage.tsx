...
export default function HomePage() {
  return (
    <div>
      <h1 className="text-xl font-bold text-gtext mb-1">盘账助手</h1>
      <p className="text-xs text-gsub mb-4">2026年4月 · 第 11 天</p>

      {/* 卡片1: 财务概览 */}
      <Card title="财务概览">
        <div className="mb-3">
          <div className="text-xs text-gsub mb-1">理财总额</div>
          <CurrencyDisplay value={totalInvest} size="xl" className="text-gblue" />
        </div>
        <StatRow
          label="净资产"
        ...
        />
        <div className="mt-3">
          <div className="flex justify-between text-xs text-gsub mb-1">
            <span>本月进度</span>
      ...
      <Card title="月度快照" subtitle="近期均值">
        ...
        />
        <div className="h-px bg-gborder my-2" />
        <div className="text-xs text-gsub mb-1">场景日均</div>
      ...
      <Card title="FIRE 提前退休" subtitle="4% 法则">
        <div className="mb-3">
          ...
          <div className="h-2.5 bg-gray-100 rounded-full overflow-hidden">
            <div
              className="h-full bg-gradient-to-r from-gblue to-teal-500 rounded-full"
        ...
        <StatRow
          label="月需存入"
          value={<CurrencyDisplay value={monthlyNeeded} className="text-orange-500" />}
        />
        <StatRow
          label="当前月结余"
          value={
        ...
        />
        <div className="h-px bg-gborder my-2" />
        <div className="text-xs text-gsub mb-1">人生进度 {age}/{lifeExpectancy}</div>
          ...
          {investKeys.map((k) => {
            ...
            return (
              <div key={k} className="flex items-center gap-2 text-xs">
                ...
                />
                <span className="text-gtext w-10">{investMeta[k].label}</span>
                <span className="tabular-nums text-gsub flex-1">
                  ¥{formatCurrency(amount)}
                </span>
                <span className="tabular-nums text-gsub w-12 text-right">
                  {(currentPct * 100).toFixed(1)}%
                </span>
                <span className={`tabular-nums w-14 text-right font-medium ${diffColor}`}>
