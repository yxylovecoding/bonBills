...
const OLD_LIFE_PERIOD_HELP_TEXT = [
  ...
  persist(
    ...
    }),
    {
      ...
      merge: (persisted, current) => {
        const p = persisted && typeof persisted === 'object' ? persisted as Partial<PrefsStore> : {};
        const lifePeriodHelpText = p.lifePeriodHelpText === OLD_LIFE_PERIOD_HELP_TEXT
          ? DEFAULT_LIFE_PERIOD_HELP_TEXT
          : p.lifePeriodHelpText;
        return { ...current, ...p, lifePeriodHelpText: lifePeriodHelpText ?? current.lifePeriodHelpText };
      },
    },
  ),
);
