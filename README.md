...
拖入图片或从邮箱获取图片附件时，会在浏览器本地通过 Tesseract OCR 识别，并按内容自动判断资产页或理财页。账户截图按“生活=建设银行+微信、收入=收入栏、理财现金中国银行=境内/嘉信=境外”映射，理财截图金额按 now/当前市值写入。首次使用会下载 OCR worker 与中文语言包，之后由浏览器缓存。
