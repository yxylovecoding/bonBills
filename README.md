...
记录页的“邮箱导入”会通过服务端 IMAP 拉取最近匹配 `账单_数字.xls/xlsx` 的附件，然后复用前端账单导入规则；Excel 预览仅展示账户和理财操作变动。如果最新附件是图片，则自动走本地 OCR 并按内容判断资产页或理财页。
...
日历中的进行中及未来出游可以自动同步到 TickTick。首次使用时，在“本月出游”中粘贴 TickTick 网页端“设置 → 账户 → API Token”生成的个人令牌。同步会读取“玩”清单中的 `出门todo` 任务树作为动态模板，并以其中带日期的 `出门当天` 对齐行程开始日。

API Token 使用 `SYNC_SECRET` 派生密钥加密后保存在服务端；修改行程或模板后会自动更新对应任务树，取消尚未结束的行程时会同步删除。

TickTick 中 `在家routine` 和 `在校routine` 父任务树内未完成且已设日期的后代任务，会自动对齐当前或下一段在家/在校日期；“学”和“实习”都视为在校。日历中找不到下一段时，暂定到下一个春节。
