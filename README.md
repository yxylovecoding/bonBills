...
日历中的进行中及未来出游可以自动同步到 TickTick。首次使用时，在“本月出游”中粘贴 TickTick 网页端“设置 → 账户 → API Token”生成的个人令牌。同步会读取“玩”清单中的 `出门todo` 任务树作为动态模板，并以其中带日期的 `出门当天` 对齐行程开始日；父任务及所有模板子任务都会使用“出行名称 · 原任务名”命名。

出游候选标签仅会被当前仍存在的其他行程占用；已删除或重新切分行程留下的历史关联不会隐藏候选项。

已启用、设有截止日且未关联出游的心愿，以心愿截止日作为预计出发日，只同步 `出门todo` 中的 `出门前七个月`（兼容 `出门前7个月`）及其后代任务。阶段根任务定在截止日前七个自然月，月底不足对应日时取当月末；已有日期的子任务和 checklist 按该阶段模板相对平移，无日期的子任务（含多层后代）继承七个月阶段日期，完整出游同步也适用。旧实例同步时补齐遗漏日期，保留手动改期或清空日期。任务名称包含心愿名，按心愿 ID 更新改名和改期，保留完成状态；关联出游、停用或删除心愿后清理尚未到截止日的独立副本，日历完整出游任务不受影响。无截止日的心愿不生成，缺少唯一七个月模板时会报错。

出游和心愿准备任务以手动日期优先：在 TickTick 中改过任务或 checklist 项的日期、时间或清空日期后，后续同步保留该项的手动安排；没改过的任务仍按模板跟随行程或心愿截止日。手动日期是固定日期，不再随出发日平移；只影响改过的项，不会锁定整棵任务树。已被旧版同步覆盖的日期需要重新调整一次。此规则不改变在家/在校 routine 的场景排期。
